...
Current request: Replace the small setup button with a brief, diegetic operation-launch overlay so the required Web Audio click feels like part of the game.
...
Completed:
- Added a full-screen `작전 개시` launch overlay with initialization, success, and retry states.
- Moved AudioSystem initialization ahead of WebGPU initialization so AudioContext creation occurs directly within the user gesture.
